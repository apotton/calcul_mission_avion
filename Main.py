...
class App(ctk.CTk):
    ...
    def check_avion(self):
        ...
        if nom_avion and nom_moteur and nom_avion != "" and nom_moteur != "":
            ...
            if not(path_avion and path_moteur):
    ...
    def load_avion(self):
        ...
        self.Avion = Avion(self.Inputs, path_avion, path_moteur)
        print(f">> Avion initialisé en mémoire.\n   - Modèle: {nom_avion}\n   - Moteur: {nom_moteur}\n")

    def calculer_mission(self):
        # Vérification présence Avion
        if not(self.check_avion()):
            return
        ...
        try:
            self.Inputs.loadCSVFile(str(chemin_csv))
        except Exception as e:
        ...
        try:
            ...
            Mission.Principal(self.Avion, self.Atmosphere, self.Enregistrement, self.Inputs)
    ...
    def calculer_pp(self):
        ...
        try:
            self.Inputs.SpeedType = self.vars["SpeedType"].get()
            self.Inputs.Speed = float(self.vars["Speed"].get())
            self.Inputs.altPP_ft = float(self.vars["altPP_ft"].get())
            self.Inputs.massPP = float(self.vars["massPP"].get())
            self.Inputs.DISA_PP = float(self.vars["DISA_PP"].get())
        except ValueError:
        ...
        try:
            resultat = PointPerformance.Performance(self.Avion, self.Atmosphere, self.Inputs)
            print(resultat)
        except Exception as e:
